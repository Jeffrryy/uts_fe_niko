...
const Listmenu = () => {
  ...
  const [newItem, setNewItem] = useState({ details: '', listMenuId: '', customerId: '' });
  ...
  const handleSubmit = async () => {
    try {
      const response = await axios.post('http://localhost:3000/listmenu/create', newItem);
      setFetchedItems([...fetchedItems, response.data]);
      handleClose();
    } catch (error) {
      console.error('Error adding list menu:', error);
    }
  };

  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:3000/listmenu/delete/${id}`);
      setFetchedItems(fetchedItems.filter(item => item.id !== id));
      setConfirmOpen(false);
    } catch (error) {
      console.error('Error deleting list menu:', error);
    }
  };
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('http://localhost:3000/listmenu/findall');
  ...
  return (
    <div className="container mx-auto">
      ...
      <div className=" flex justify-between mb-6">
        <h1 className="text-2xl font-bold text-warna-pecel flex justify-center">List Menu</h1>
        <button
          className="bg-green-500 text-white px-4 py-2 rounded hover:bg-green-600 ml-4 justify-center"
          onClick={handleClickOpen}
        > 
         Tambah List Menu Baru <AddIcon />
        </button>
      </div>
      ...
      <Dialog open={open} onClose={handleClose}>
        <DialogTitle>Tambah List Menu Baru</DialogTitle>
        <DialogContent>
          <TextField
            autoFocus
            margin="dense"
              ...
              <div className="sm:flex sm:items-start">
                ...
                <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                  <h3 className="text-lg leading-6 font-medium text-gray-900" id="modal-title">
                    Konfirmasi Hapus List Menu
                  </h3>
                  <div className="mt-2">
                    <p className="text-sm text-gray-500">
                      Apakah Anda yakin ingin menghapus list menu ini? Tindakan ini tidak dapat dibatalkan.
                    </p>
                  </div>
                </div>
              </div>
